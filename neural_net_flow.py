...
def get_callback():
    ...
    class MetaflowCardUpdates(callbacks.Callback):
        def on_train_batch_begin(self, batch, logs=None):
            ...
            print("Value of logs on_train_batch_begin: ",batch, logs)
        
        def on_train_batch_end(self, batch, logs=None):
            print("Value of logs on_train_batch_end: ",batch, logs)
        
        def on_test_batch_begin(self, batch, logs=None):
            print("Value of logs on_test_batch_begin: ",batch, logs)
        
        def on_test_batch_end(self, batch, logs=None):
            print("Value of logs on_test_batch_end: ",batch, logs)
        
        def on_epoch_begin(self, epoch, logs=None):
            print("Value of logs on_epoch_begin: ",epoch, logs)

        def on_epoch_end(self, epoch, logs=None):
            print("Value of logs on_epoch_end: ", epoch,logs)
...
class NeuralNetCardFlow(FlowSpec):
    ...
    batch_size = Parameter('batch-size', default=128)
    ...
    def start(self):
        ...
        self.y_train = utils.to_categorical(
            y_train, self.num_classes)
        self.y_test = utils.to_categorical(
